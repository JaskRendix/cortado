...
public class KateOverlay extends Overlay {
  ...
  private final Pad kateSinkPad =
      new Pad(Pad.SINK, "katesink") {
        @Override
        ...
        /**
         * This pad receives Kate events, and add them to the renderer. They will be removed from it
  ...
  /**
   * Add a new Kate event to the renderer. This needs locking so the Kate events are not changed
   * while the overlay is rendering them to an image.
   */
  ...
  /**
   * Upon a flushing event, remove any existing event, now obsolete. This needs locking so the Kate
   * events are not changed while the overlay is rendering them to an image.
   */
