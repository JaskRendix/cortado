...
public final class Quant {
    ...
    for (int coding = 0; coding < 2; ++coding) {
      for (int plane = 0; plane < 3; ++plane) {
        for (int quality = 0; quality < 64; ++quality) {
          short[] scaledmat =
              compQuantMatrix(
                  ci.AcScaleFactorTable,
                  ci.DcScaleFactorTable,
                  ci.qmats,
                  nqrs,
                  qrsizes,
                  qrbmis,
  ...
  static short[] compQuantMatrix(
    ...
    for (int ci = 0; ci < 64; ++ci) {
      bm[ci] =
          (2 * (qiend - qi) * bms[(bmi << 6) + ci]
                  + 2 * (qi - qistart) * bms[(bmj << 6) + ci]
                  + qrsizes[qti][pli][qri])
              / (2 * qrsizes[qti][pli][qri]);

      if (ci == 0 && qti == 0) qmin = 16;
      else if (ci > 0 && qti == 0) qmin = 8;
      else if (ci == 0 && qti == 1) qmin = 32;
      else qmin = 16;
