    ...
    for (int j = 0; j < 3; j++) {
      ...
      for (int n = 1; n < fragsAcross - 1; n++) {
        if (pbi.display_fragments[i] != 0) {
          ...
          filterHoriz(pbi.LastFrameRecon, index - 2, lineLength, filtBoundingValue);

          /* Filter right hand border only if the block to the right is
          not coded */
          if (pbi.display_fragments[i + 1] == 0) {
            filterHoriz(pbi.LastFrameRecon, index + 6, lineLength, filtBoundingValue);
          }
      ...
      if (pbi.display_fragments[i] != 0) {
        /* Filter Left edge always */
        filterHoriz(
            pbi.LastFrameRecon, pbi.recon_pixel_index_table[i] - 2, lineLength, filtBoundingValue);
      ...
      for (int m = 1; m < fragsDown - 1; m++) {
        ...
        if (pbi.display_fragments[i] != 0) {
          ...
          filterVert(pbi.LastFrameRecon, index, lineLength, filtBoundingValue);
