...
/** Handles the decoding state of a Kate stream. */
public class State {
  ...
  public int decodeInit(Info ki) {
    if (ki == null) {
      return Result.KATE_E_INVALID_PARAMETER;
    }
    dec = new Decode(ki);
    granulepos = -1;
    return 0;
  ...
  /**
   * Decode a Kate data packet. Headers are supposed to have been parsed already. An event may be
   * generated, and will then be available from decodeEventOut.
   */
  public int decodePacketin(Packet op) {
    if (op == null || dec == null || dec.opb == null) {
      return Result.KATE_E_INVALID_PARAMETER;
    }
    ...
    dec.opb.readInit(op.packetBase, op.packet, op.bytes);

    /* get packet type */
    type = dec.opb.read(8);

    /* ignore headers */
    if ((type & 0x80) != 0) return 0;

    switch (type) {
      case 0x00:
        if (dec.info == null) {
          return Result.KATE_E_INIT;
        }
        ...
        if (op.granulepos > -1) granulepos = op.granulepos;
        else {
          if (granulepos == -1) {
            granulepos = 0;
          }
