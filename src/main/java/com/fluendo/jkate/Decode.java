...
public final class Decode {
  ...
  /**
   * Decode a text data packet (packet type 0x00), and fills the passed event from the decoded data.
   */
  public int decodeTextPacket(Event ev) {
    if (ev == null) {
    ...
    if (opb.read1() != 0) {
      ...
      for (int n = 0; n < nmotions; ++n) {
        if (opb.read1() != 0) {
          ...
          if (info.motions == null || idx < 0 || idx >= info.motions.length) {
            return -1;
          }
    ...
    if (opb.read1() != 0) {
      ...
      if (opb.read1() != 0) {
        int idx = Bitwise.read32v(opb);
        if (info.regions == null || idx < 0 || idx >= info.regions.length) {
          return Result.KATE_E_BAD_PACKET;
        }
      ...
      if (opb.read1() != 0) {
        int idx = Bitwise.read32v(opb);
        if (info.styles == null || idx < 0 || idx >= info.styles.length) {
          return Result.KATE_E_BAD_PACKET;
        }
      ...
      if (opb.read1() != 0) {
        int idx = Bitwise.read32v(opb);
        if (info.font_mappings == null || idx < 0 || idx >= info.font_mappings.length) {
          return Result.KATE_E_BAD_PACKET;
        }
    ...
    if (((info.bitstream_version_major << 8) | info.bitstream_version_minor) >= 0x0002) {
      ...
      if (opb.read1() != 0) {
        if (opb.read1() != 0) {
          int idx = Bitwise.read32v(opb);
          if (info.palettes == null || idx < 0 || idx >= info.palettes.length) {
            return Result.KATE_E_BAD_PACKET;
          }
        ...
        if (opb.read1() != 0) {
          int idx = Bitwise.read32v(opb);
          if (info.bitmaps == null || idx < 0 || idx >= info.bitmaps.length) {
            return Result.KATE_E_BAD_PACKET;
          }
  ...
  public double granuleTime(long granulepos) {
    if (granulepos >= 0 && info.gps_numerator != 0) {
      long base = granulepos >> info.granule_shift;
      long offset = granulepos - (base << info.granule_shift);

      return (base + offset) * ((double) info.gps_denominator / info.gps_numerator);
    }
    return -1;
  }
  ...
  public double granuleDuration(long granule) {
    if (granule >= 0 && info.gps_numerator != 0) {
      return granule * ((double) info.gps_denominator / info.gps_numerator);
    }
    return -1;
  }
}
