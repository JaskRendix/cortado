...
public class CortadoPipeline extends Pipeline implements PadListener, CapsListener {
  ...
  public void padAdded(Pad pad) {
    ...
    switch (mime) {
      ...
      case "image/jpeg" -> {
        if (!enableVideo) return;
        if (!setupVideoDec("jpegdec")) return;
      ...
      case "video/x-smoke" -> {
        if (!enableVideo) return;
        if (!setupVideoDec("smokedec")) return;
        videodec.setProperty("component", component);
        pad.link(videodec.getPad("sink"));
        if (!videodec.getPad("src").link(ovsinkpad)) {
          postMessage(Message.newError(this, "videosink already linked"));
      ...
      case "application/x-kate" -> {
        ...
        if (enableKate == katedec.size() - 1) {
          ...
          boolean matchingLanguage =
              enableKateLanguage.isEmpty() || enableKateLanguage.equals(language);
  ...
  public void padRemoved(Pad pad) {
    ...
    if (pad == vpad) {
      ...
      if (ovsinkpad != null) ovsinkpad.unlink();
      vpad = null;
    } else if (pad == apad) {
      Debug.log(Debug.INFO, "audio pad removed " + pad);
      if (asinkpad != null) asinkpad.unlink();
      apad = null;
    }
  }
    ...
    if (!language.isEmpty() || !category.isEmpty()) {
      for (int n = 0; n < katedec.size(); ++n) {
        ...
        if (e != null) {
          ...
          if (language.equalsIgnoreCase(eLanguage)
              && (category.isEmpty() || category.equals(eCategory))) {
            return n;
          }
        }
      }
  ...
  public void resize(Dimension d) {
    if (videosink == null || d == null) return;
  ...
  public boolean buildOgg() {
    ...
    if (bufferSize != -1) buffer.setProperty("maxSize", bufferSize * 1024);
    if (bufferLow != -1) buffer.setProperty("lowPercent", bufferLow);
    if (bufferHigh != -1) buffer.setProperty("highPercent", bufferHigh);
    add(demux);
    add(buffer);
  ...
  private boolean build() {
    ...
    if (enableVideo) {
      ...
      videosink.setProperty(
          "max-lateness", String.valueOf(enableAudio ? Clock.MSECOND * 20 : Long.MAX_VALUE));
      add(videosink);
