...
public class CortadoPipeline extends Pipeline implements PadListener, CapsListener {
  ...
  // Modernization: Replaced legacy java.util.Vector with modern java.util.List
  ...
  public void padAdded(Pad pad) {
    ...
    // Modernization: Replaced if-else chains with Java 14+ switch expressions
    switch (mime) {
      case "audio/x-vorbis" -> {
        if (!enableAudio)
          return;
      ...
      case "application/x-kate" -> {
        ...
        katedec.add(tmp_katedec);
        k_queue.add(tmp_k_queue);
        ...
        if (enableKate == katedec.size() - 1) {
          ...
          boolean matching_language = enableKateLanguage.isEmpty() || enableKateLanguage.equals(language);
          boolean matching_category = enableKateCategory.isEmpty() || enableKateCategory.equals(category);
      ...
      for (int n = 0; n < katedec.size(); ++n) {
        ...
        if (e != null) {
          ...
          if (language.equalsIgnoreCase(e_language)) {
            if (category.isEmpty() || category.equals(e_category)) {
              return n;
            }
          }
        }
  ...
  private void doEnableKateIndex(int idx) {
    if (kselector != null) {
      ...
      // Modernization: Utilized autoboxing instead of Integer.valueOf()
  ...
  private boolean build() {
    ...
    String userAgent = "Cortado/1.2.0 " +
        (vendor != null && vendor.contains(" ") ? vendor.substring(0, vendor.indexOf(" ")) : "Java") + "/" +
    ...
    try {
      ...
      // Ignored specifically for security exception scenarios reading system props
    }
    ...
    if (enableVideo) {
      ...
      // Boolean values stringified as they are likely expected as strict Strings in
      // this older pipeline property config
