...
class Floor1 extends FuncFloor {
  ...
  Object unpack(Info vi, Buffer opb) {
    ...
    for (int j = 0; j < maxclass + 1; j++) {
      ...
      if (info.class_book[j] < 0 || info.class_book[j] >= vi.getBooks()) {
        info.free();
        return null;
      }
      for (int k = 0; k < (1 << info.class_subs[j]); k++) {
        info.class_subbook[j][k] = opb.read(8) - 1;
        if (info.class_subbook[j][k] < -1 || info.class_subbook[j][k] >= vi.getBooks()) {
          info.free();
          return null;
        }
      }
  ...
  Object inverse1(Block vb, Object ii, Object memo) {
    ...
    if (vb.opb.read(1) == 1) {
      ...
      if (fitValue == null || fitValue.length < look.posts) {
        ...
        for (int i = 0; i < fitValue.length; i++) fitValue[i] = 0;
      }

      fitValue[0] = vb.opb.read(ilog(look.quant_q - 1));
      fitValue[1] = vb.opb.read(ilog(look.quant_q - 1));
      ...
      for (int i = 0, j = 2; i < info.partitions; i++) {
        ...
        for (int k = 0; k < cdim; k++) {
          ...
          cval >>>= csubbits;
          if (book >= 0) {
            if ((fitValue[j + k] = books[book].decode(vb.opb)) == -1) {
              return null;
            }
          } else {
            fitValue[j + k] = 0;
  ...
  int inverse2(Block vb, Object i, Object memo, float[] out) {
    ...
    InfoFloor1 info = look.vi;
    int n = vb.vd.vi.getBlocksizes()[vb.mode] / 2;
  ...
  private static final float[] FLOOR_FROM_DB_LOOKUP = {
