        ...
        while (true) { // we repeat if the bitstream is chained
            ...
            // Throw the comments plus a few lines about the bitstream we're decoding
            {
                ...
                for (int j = 0; j < ptr.length; j++) {
                    if (ptr[j] == null) break;
                    System.err.println(new String(ptr[j], 0, ptr[j].length - 1, StandardCharsets.UTF_8));
                } 
                System.err.println("\nBitstream is " + vi.getChannels() + " channel, " + vi.getRate() + "Hz");
                System.err.println("Encoded by: " + new String(vc.vendor, 0, vc.vendor.length - 1, StandardCharsets.UTF_8) + "\n");
            }
        
            convsize = 4096 / vi.getChannels();
            ...
            int[] _index = new int[vi.getChannels()];
                                ...
                                while ((samples = vd.synthesis_pcmout(_pcm, _index)) > 0) {
                                    ...
                                    for (i = 0; i < vi.getChannels(); i++) {
                                        ...
                                        for (int j = 0; j < bout; j++) {
                                            ...
                                            ptr += 2 * (vi.getChannels());
                                        }
                                    }
                                
                                    System.out.write(convbuffer, 0, 2 * vi.getChannels() * bout);
