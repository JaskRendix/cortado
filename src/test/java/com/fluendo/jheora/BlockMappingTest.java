...
public class BlockMappingTest {
    ...
    /**
     * Reference implementation of the mapping logic, mirroring BlockMapping#createMapping
     * and the constructor, so we can assert that the public API returns consistent values.
     */
    private int[][][] buildExpectedBlockMap(int ySuperBlocks,
                                            int uvSuperBlocks,
                                            int hFrags,
                                            int vFrags,
                                            int shiftx,
    ...
    void quadMapToIndex1_matchesReferenceImplementation_onRegularGrid() {
        ...
        BlockMapping mapping = new BlockMapping(ySuperBlocks, uvSuperBlocks, hFrags, vFrags, shiftx, shifty);
        int[][][] expected = buildExpectedBlockMap(ySuperBlocks, uvSuperBlocks, hFrags, vFrags, shiftx, shifty);
    ...
    void quadMapToMBTopLeft_returnsTopLeftFragmentIndex() {
        ...
        for (int sb = 0; sb < totalSBs; sb++) {
            for (int mb = 0; mb < 4; mb++) {
                ...
                assertEquals(expectedTopLeft, actualTopLeft,
                        "Top-left mismatch at sb=" + sb + ", mb=" + mb);
            }
        }
    }
    ...
    void mapping_handlesNonMultipleOfFourFragmentDimensions_withMinusOneForUnusedBlocks() {
        ...
        assertTrue(foundUnused,
                "Expected at least one unused block mapped to -1 for irregular dimensions");
    }
