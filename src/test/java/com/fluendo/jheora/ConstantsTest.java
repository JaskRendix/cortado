...
public class ConstantsTest {
    ...
    void blockSizeIsCorrect() {
        assertEquals(64, Constants.BLOCK_SIZE,
                "BLOCK_SIZE must be 8×8 = 64");
    }

    @Test
    void umvBorderAndStrideExtraAreCorrect() {
        assertEquals(16, Constants.UMV_BORDER);
        assertEquals(32, Constants.STRIDE_EXTRA,
                "STRIDE_EXTRA must be UMV_BORDER × 2");
    }
    ...
    void dequantIndexHasCorrectLength() {
        assertEquals(64, Constants.DEQUANT_INDEX.length,
                "DEQUANT_INDEX must contain exactly 64 entries");
    }
    ...
    void dequantIndexContainsAllValuesZeroToSixtyThreeExactlyOnce() {
        ...
        for (int v : Constants.DEQUANT_INDEX) {
            assertTrue(v >= 0 && v < 64,
                    "DEQUANT_INDEX contains out-of-range value: " + v);

            assertFalse(seen[v],
                    "Duplicate DEQUANT_INDEX value detected: " + v);
    ...
    void constantsAreImmutable() {
        ...
        assertEquals(before, Constants.DEQUANT_INDEX[0],
                "DEQUANT_INDEX must not be mutated externally");
    }
}
