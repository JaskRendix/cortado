...
public class TheoraDecTest {
    ...
    public void testTypeFindWithOffset() {
        byte[] dataWithOffset = new byte[] { 0x00, 0x00, (byte) 0x80, 0x74, 0x68, 0x65, 0x6f, 0x72, 0x61 };
    ...
    public void testIsHeaderCheck() {
        ...
        assertTrue(theoraDec.isHeader(packet), "Packet with high bit set should be recognized as header");

        packet.packet_base = new byte[] { 0x00 }; // High bit clear indicates video frame
        assertFalse(theoraDec.isHeader(packet), "Packet with high bit clear should not be recognized as header");
    }
