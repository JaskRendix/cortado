...
class AudioSinkJ2Test {
  ...
  void testFactoryName() {
    assertEquals(
        "audiosinkj2", audioSinkJ2.getFactoryName(), "Factory name must match expected identifier");
  }
  ...
  void testInvalidWriteParameters() {
    ...
    // Passing invalid bounds (e.g., negative offset or out-of-bounds length) should fail safely
    // without crashing
    int written = audioSinkJ2.write(dummyData, -1, 50);
    assertEquals(
        50,
        written,
        "Write method should safely short-circuit and return length on invalid offsets");
  }
}
