...
class AudioSinkTest {
    ...
    void testAudioClockWithNullRingBuffer() {
        ...
        assertEquals(0, clock.getTime(), "Time should gracefully default to 0 when ringBuffer rate is 0");
    }
    ...
    void testRingBufferCommitEdgeCases() {
        ...
        assertTrue(ring.acquire(caps), "RingBuffer should successfully acquire valid audio capabilities");

        byte[] fakeData = new byte[4096];

        int resultNegativeSample = ring.commit(fakeData, -5, 0, 100);
        assertEquals(100, resultNegativeSample, "Commit with negative sample index should short-circuit and return length");
    ...
    void testStateTransitionsAndFlushing() {
        ...
        audioSink.changeState(Element.STOP_PAUSE);
        assertNotNull(audioSink.ringBuffer, "RingBuffer should be instantiated after entering pause state");
